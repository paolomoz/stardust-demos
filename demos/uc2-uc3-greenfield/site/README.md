...
1. **Brand definition** (`brand/BRAND.md`) — locked.
2. **Content briefing** (`briefing/`) — direction for each page on the site.
3. **Logo + brand imagery** (`assets/`) — generated externally with Gemini 3 Pro Image Preview, dropped in here.
4. **Placeholder site** (`current/`) — thin static HTML with the brand assets, to be crawled by `/stardust:extract` (UC2's "no source site" workaround per `context/use-cases.md`).
5. **Stardust direction** (`/stardust:direct`) — produces `target/PRODUCT.md`, `target/DESIGN.md`, `target/DESIGN.json` from a short intent phrase + the placeholder.
